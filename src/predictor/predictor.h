...
struct PredictorConfig {
    ...
    std::string ai_model = "gpt-5-mini";
...
public:
    ...
    // Tahminleri al — trie ön ek araması + ngram bağlam birleşimi
    std::vector<Candidate> predict(const std::string& prefix) const;

    // Get next-word predictions (after space, no prefix yet)
    // Sonraki kelime tahminleri (boşluktan sonra, henüz ön ek yok)
    ...
    // Boşluk/enter basıldığında — buffer'ı last_word'e taşır
...
private:
    ...
    // Kullanıcı kelime frekansı — en çok kullanılan kelimeler öncelikli
